...
/*
 * :file description:
 * :name: /xiaozhicloud-pc/src/pages/digit/index.tsx
 * :author: 张德志
 * :copyright: (c) 2023, Tungee
 * :date created: 2023-04-26 01:37:22
 * :last editor: 张德志
 * :date last edited: 2023-10-27 06:29:05
 */
...
import { getDigitList, getDigitDelete } from './service';
...
const Website: React.FC = () => {
  ...
  const fetchMetaverseList = async (params: any) => {
    const res = await getDigitList(params);
  ...
  const handleConfirm = async (id: string) => {
    const res = await getDigitDelete({ _id: id });
    if (res.code === SUCCESS_CODE) {
